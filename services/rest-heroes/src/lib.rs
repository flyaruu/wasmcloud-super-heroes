...
impl Guest for HeroFetcher {
    fn handle(_request: IncomingRequest, response_out: ResponseOutparam) {
        ...
        let resp = a.first().unwrap().first().unwrap();
        let val = match &resp.value {
            PgValue::Varchar((s,y)) => y,
        ...
        write_stream.write("whoop".as_bytes()).unwrap();
        // serde_json::to_writer(write_stream, &heroes).unwrap();

        // write_stream.write_all(val.as_bytes()).unwrap();
        drop(write_stream);
